...
public class Student extends Person {
	...
	public Student(String name, int egn, String university, String speciality, int facNumber, int age, int height,
			int weight, String type) {
		super(name, egn, age, height, weight, facNumber, type);
		this.university = university;
		this.speciality = speciality;
		this.facNumber = facNumber;
	}
	...
	/*
	 * Пренаписване на метод от клас родител. В runtime всички обекти от тип Student
	 * ще извикват само пренаписаният метод, дори и да са дефинирани като обекти от
	 * базовият тип
	 */
	@Override
	public void run() {
	...
	public String toString() {
		return String.format("The student %s is learning in %s speciality", this.getName(), this.getSpeciality());
	}
	...
	// имплементиран в абстрактният клас. Ако е необходимо getShower() може да се
	// пренапишe)
	...
	// Добавяме в класа Student помощен клас за произволно създаване на студенти -
	// StudentGenerator
	public static class StudentGenerator {
		...
		public static Student make() {
			...
			return new Student(name, egn, university, speciality, facNumber, age, height, weight, type);
		}
	}
}